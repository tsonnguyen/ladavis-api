	...
	.on('end', function () {
		...
		for (var l = 0; l < numOfDataSet; l++) {
			...
			for (var j = 0; j < numOfTraining; j++) {
				...
				var svm = new ml.SVM({ x : dataArray.slice(0, 768 - 230), y : labelArray.slice(0, 768 - 230) });
				svm.train({
				...
				for (let i = 768 - 230; i < 768; i++) {
					var result = svm.predict(dataArray[i]);
				...
				if (print) { finalClusters = svm; }
			}
		}

		fs.writeFile('/svm', JSON.stringify(finalClusters), function(err) {
