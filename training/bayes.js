	...
	.on('end', function () {
		...
		for (var l = 0; l < numOfDataSet; l++) {
			...
			for (var j = 0; j < numOfTraining; j++) {
				...
				for (let i = 0; i < 768 - 230; i++) {
					bayesClassifier.learn(dataArray[i], labelArray[i]);
				}
				...
				if (print) { finalClusters = bayesClassifier.toJson(); }
			}
		}

		fs.writeFile('/bayes', JSON.stringify(finalClusters), function(err) {
